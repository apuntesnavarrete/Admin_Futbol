...
const plantel_1 = await pool.query("SELECT * FROM `agui_general_libre_c22` WHERE Equipo = ?",[equipo]);
const plantel_2 = await pool.query("SELECT * FROM `agui_general_libre_c22` WHERE Equipo = ?",[equipo_2]);
...
router.post('/actAlibre' , async (req,res,next)=>{
...
await pool.query("INSERT INTO agui_libre_c22 set ?",[Resul_1])
await pool.query("INSERT INTO agui_libre_c22 set ?",[Resul_2])
...
res.redirect('http://localhost:8081/resul/agui/libre')
})
