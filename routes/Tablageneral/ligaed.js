...
router.get('/mix', async(req, res, next)=> {
  const vistas = await pool.query("SELECT * FROM `ed_general_mix_c22`");
  const categoria = 'Mixta C-2022';

  const result = await pool.query("SELECT * FROM `ed_jor_mix_c2022`");
...
vistas_ganadospenales = vistas_ganadospenales.reverse()
  ...
  router.get('/fem', async(req, res, next)=> {
